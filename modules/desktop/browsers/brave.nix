...
in
{
  options.modules.desktop.browsers.brave =
    ...
    in
    {
      enable = mkEnableOption "Google-free brave";
    };
  ...
  config = mkIf config.modules.desktop.browsers.brave.enable {
    user.packages =
      ...
      [
        (makeDesktopItem {
          ...
          genericName = "Launch a Private Brave Instance";
          icon = "brave";
          exec = "${getExe brave}/bin/brave --incognito";
          categories = [ "Network" ];
        })
      ];
    ...
    hm.programs.chromium = {
      ...
      package =
        ...
        in
        pkgs.brave.override { commandLineArgs = [ ungoogledFlags ]; };
      extensions = [
